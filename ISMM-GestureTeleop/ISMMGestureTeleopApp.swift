...
class ISMMGestureTeleopApp: NSObject, GestureRecognizerLiveStreamDelegate {
    ...
    private func sendCommand(_ command: TeleopCommand, pose: Pose?) {
        ...
        // --- LOGGING vs SENDING SPLIT ---
        
        // 1. Capture common timestamp
        let timestamp = Date().timeIntervalSince1970
        
        // 2. Log to CSV: Use the struct so the Logger handles the CSV formatting
        let logEntry = TeleopMessage(timestamp: timestamp, payload: message)
        logger.log(packet: logEntry)
        
        // 3. Send to Robot: Convert the RAW STRING to data (No JSON, no Timestamp)
        // Note: Check if your robot needs a newline "\n" at the end.
        // If so, use: (message + "\n").data(using: .utf8)
...
class PacketLogger {
    ...
    init() {
        print("PacketLogger: Init started...") // 1. Sanity check
        
        let fileName = "teleop_log_\(Int(Date().timeIntervalSince1970)).csv"
        
        // Check Document Directory
        guard let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
            print("PacketLogger: Failed to find Documents Directory!") // 2. Silent failure check
            return
        }
        
        let fileURL = docDir.appendingPathComponent(fileName)
        print("PacketLogger: Target URL is \(fileURL.path)") // 3. verify path
        
        let header = "Timestamp_Unix,Command_Payload\n"
        
        do {
            // Write the header (Creates the file)
            try header.write(to: fileURL, atomically: true, encoding: .utf8)
            print("PacketLogger: File created successfully.") // 4. File creation check
            
            // Open the handle
            self.fileHandle = try FileHandle(forWritingTo: fileURL)
            self.fileHandle?.seekToEndOfFile()
            print("PacketLogger: Ready to log.") // 5. Success
            
        } catch {
            print("PacketLogger Error: \(error)") // 6. Exception check
        }
    }
    
    func log(packet: TeleopMessage) {
        guard let fileHandle = fileHandle else { return }
        
        // CSV Formatting Logic:
        // 1. Double up any existing quotes in the JSON (Standard CSV escape rule)
        let escapedCommand = packet.payload.replacingOccurrences(of: "\"", with: "\"\"")
        
        // 2. Wrap the whole command in quotes so commas inside JSON don't break the CSV columns
        let logLine = String(format: "%.6f,\"%@\"\n", packet.timestamp, escapedCommand)
